...
class GrUser extends AppModel
{
    public  $useTable = "gr_users";
    public $pagesize=0;
    public $pagenow=1;
    ...
    public function addgruser(array $data){
//        $data['pub_time']=date("Y-m-d H:i:s");
    ...
    public function getlist($where='',$etc='',$field=''){
        ...
        if ($field){
            $sql="select $field from gr_users WHERE 1".$where.$etc;
        }else{
            $sql="select * from gr_users WHERE 1".$where.$etc;
        }
