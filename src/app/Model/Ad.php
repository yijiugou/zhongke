...
{
    public  $useTable = "adlist";
    public $pagenow=0;
    public $pagesize=10;
     //资讯分享查询数量
    public function count($category = 0){
        if ($category == 0) {
            $count = "select count(1) from adlist";
        } else {
            $count = "select count(1) from adlist where class=" . $category;
        }
        return $this->getCount($count, 'adlist', 'id');
    }
    ...
    public function getArticlesById($id){
        $sql = "select * from adlist where id=" . $id;
        return $this->getAll($sql);
    }
    //根据ID查询该文章的下一条数据
    public function getNextById($id, $category){
        $sql = 'select id,title from adlist where id > ' . $id . ' and class=' . $category . ' ORDER BY id ASC LIMIT 0,1';
        return $this->getAll($sql);
    }
    //根据ID查询该文章的下一条数据
    public function getPrevById($id, $category){
        $sql = 'select id,title from adlist where id < ' . $id . ' and class=' . $category . ' ORDER BY id DESC LIMIT 0,1';
        return $this->getAll($sql);
    }
    /**
     * 获取文章列表 根据分类
    ...
    public function getArticle($where='',$etc='',$field=''){
        ...
        if ($field){
            $sql="select $field from adlist WHERE 1".$where.$etc;
        }else{
            $sql="select * from adlist WHERE 1".$where.$etc;
        }
        return $this->getAll($sql);
    }
    ...
    public function articleDelete($id){
        $sql="delete from adlist WHERE id = ".$id;
