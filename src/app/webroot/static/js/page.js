...
var pageInit = {
		...
		setErmbox: function (obj, title) {
			obj.click(function () {
				var str =
					'<div class="ermsblack"><div class="ermSBox"><div class="img"><img src="' +
					obj.attr("data-img") +
					'"/></div><div class="t">' +
					title +
					"</div></div></div>";
		...
		pbanner: function () {
			if (jQuery(".pbanner").length >= 1) {
				if (!isMobile) {
					jQuery(".pbanner").css(
						"height",
						jQuery(".pbanner .load-img").height()
					);
				} else {
	...
	pbanner = {
		init: function () {
			if (jQuery(".load-img").length >= 1) {
				_PreLoadImg([jQuery(".load-img").attr("src")], function () {
					pageInit.pbanner();
				});
...
jQuery(window).scroll(function () {
	...
	var icheight =
		$(".footerbg").offset().top - win_height + $(".footerbg").height() - 10;
	if (windowWidth > 1200) {
